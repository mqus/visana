...
from tkinter.ttk import Combobox, LabelFrame, Checkbutton
...
class SimpleScatter(Frame):
    ...
    def cluster_changed(self, in_table):
        ...
        self.settings=SSControls(self,self.window.calc.get_all_columns(with_time=True))
        self.settings.grid(column=0, row=0, sticky=(S,W,N,E))
    ...
    def draw_plot(self):
        ...
        if c is not None:
            for i in range(len(c)):
                ...
                x = d2[self.param_x]
                y = d2[self.param_y]
                self.ax.scatter(x=x, y=y, marker="o",color=COLORS[i], s=4,
                                alpha=0.2, linewidths=0, picker=self.handle_pick)
        else:
            self.ax.scatter(x=x_all, y=y_all, marker="o",color="black", s=4, alpha=0.2,linewidths=0,
                                  picker=self.handle_pick)

        self.ax.set_xlabel(self.param_x)
        self.ax.set_ylabel(self.param_y)
        #widen plot box by 2% on ech side
        padding=0.02# 2%

        xmin = x_all.min() - padding*(x_all.max()-x_all.min())
        xmax = x_all.max() + padding*(x_all.max()-x_all.min())
        ymin = y_all.min() - padding*(y_all.max()-y_all.min())
        ymax = y_all.max() + padding*(y_all.max()-y_all.min())
...
class SSControls(LabelFrame):
    def __init__(self, parent, params):
        ...
        param1box=Combobox(self, textvariable=self.param1var, state="readonly")
        param1box['values'] = self.params
        param1box.bind('<<ComboboxSelected>>', self.parent.params_changed)
        param1box.grid(column=1, row=1, sticky=(N, E, W))
        ...
        param2box = Combobox(self, textvariable=self.param2var, state="readonly")
        param2box['values'] = self.params
        param2box.bind('<<ComboboxSelected>>', self.parent.params_changed)
        param2box.grid(column=1, row=2, sticky=(N, E, W))

        #Regression
        self.rgvar = StringVar(value="0")
        self.regression = Checkbutton(self, text='Draw Regression', command=self.parent.params_changed,
                                         variable=self.rgvar)
